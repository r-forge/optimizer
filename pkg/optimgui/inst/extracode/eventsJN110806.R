...
# JN
# Choose method event 
# from  addHandlerClicked(chooseButton, onChooseButton, param);
onChooseButton = function(h, ...)
{
    # Error conditions:
    #  - no objective
    #  - no Rop specified (possibly cannot get here in that case)
    # Get status (obj, grad, hessian, bounds, parameters
 
    # get methods available
      if (! require("optimx") ) { # only optim available
      }
      # if (! require( < other methods eventually > ))
      # use a table in form of package:function that has entries for different needs. Should match catalog.
    editor = h$action$editor; # ?? not sure we need this
    tmpfile = tempfile();
    visible(outputWidget) = TRUE;
	svalue(outputWidget) = paste(svalue(outputWidget), output, "\n", sep = "");
    svalue(editor$noteBook) = which(names(editor$tabsList) == "Run") + 1;
