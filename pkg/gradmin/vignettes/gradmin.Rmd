...
There is a huge number of methods for function minimization based on the use of gradients of 
the function to be minimized. This vignette presents **gradmin**, an **R** function that 
allows many, possibly most, such algorithms to be tried. 
...
There follows an iteration of the following three components:
  ...
  - the **line search**, which chooses a particular step length according to
  some rule. 
...
**Termination** is often confused with **convergence**. The former applies to 
methods or programs, and is the way in which we stop our method. Termination
...
Thus we can had a "naive" method where we set $step = 1$, the choice of the 
traditional Newton method, but in practice,
this can sometimes mean that the function value is not reduced. Generally, 
...
but in most cases we do not require a very precise step length.
