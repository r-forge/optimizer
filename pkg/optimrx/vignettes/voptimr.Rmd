...
code to match. Package `optimrx` may also be expected to contain extra tests, 
commentary, and other material.
...
includes the function `nmkb()` from package `dfoptim`. Unfortunately, this function
uses the transfinite transformation of the objective function to impose the bounds
(ref??), which causes and error if any of the initial parameters are on one of the
bounds. 

There are several improvements in the `optimr` package that would be especially nice to
...
control of the approach. For example, the transfinite approach could be used with some of
the methods that allow bounds. Note that these possibilities increase the complexity of the
code and may be prone to bugs. 
...
`control` list in the element `parscale`. In the tests of the package, and as an example
...
It is often convenient to be able to run multimple 
...
set of triplets consisting of a method name (character), an iteration count and a function evaluation count.
...
step derivative would be useful to include in this vignette.
