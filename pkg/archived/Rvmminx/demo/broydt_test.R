...
for (ii in ni) {
   ...
   x0<-rep(pi,n)
   ut<-system.time(ans<-Rvmminstep(x0,broydt.f,broydt.g,control=list(trace=1)))[1]
   times[ii,1]<-ut
   cat("unconstrained with gradient\n")
#   ans
   optansout(ans,filename=afname)
   afname<-paste("ansbroydt",n,"UN",sep='')
   ut<-system.time(ans<-Rvmminstep(x0,broydt.f,control=list(trace=1)))[1]
   times[ii,2 ]<-ut
...
#   ans
   ...
   x0<-rep(pi,n)
   ut<-system.time(ans<-Rvmminstep(x0,broydt.f,broydt.g,lower=lower,upper=upper,control=list(trace=1)))[1]
   times[ii,3]<-ut
   cat("constrained, no analytic gradient\n")
#   ans
   optansout(ans,filename=afname)
   afname<-paste("ansbroydt",n,"BN",sep='')
   ut<-system.time(ans<-Rvmminstep(x0,broydt.f,lower=lower,upper=upper,control=list(trace=1)))[1]
   times[ii,4]<-ut
   cat("constrained, no analytic gradient\n")
#   ans
   optansout(ans,filename=afname)
