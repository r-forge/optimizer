...
/*============================================================================
  ...
  ----------------------------------------------------------------------------*/
  // Printing using printf is forbidden in R packages.
...
typedef struct cg_stats_struct /* statistics returned to user */
{
    ...
    long            IterSub ; /* number of subspace iterations */
    long             NumSub ; /* total number subspaces */
    long              nfunc ; /* number of function evaluations */
    long              ngrad ; /* number of gradient evaluations */
} cg_stats ;
...
(
    ...
    std::function<double(double *, long)> value,
    //    double        (*value) (double *, long),  /* f = value (x, n) */
    std::function<void(double *, double *, long)> grad,
    //    void           (*grad) (double *, double *, long), /* grad (g, x, n) */
    std::function<double(double *, double *, long)> valgrad,
    //    double      (*valgrad) (double *, double *, long), /* f = valgrad (g,x,n)*/
    double         *Work  /* either size 4n work array or nullptr */
) ;
