...
IBM PC family of MS DOS computers running GWBASIC. (?? Do we want to make these available?)
...
the triangles made up from vertex `1` with vertex `L` and `(L+1)`
where `L` runs from `2` to `(nv-1)`.
...
There are a number of unconstrained optimization methods, and the `optimr} package lets
...
Let us try all the bounds-constrained methods in `opm} from package `optimr.
...
- polyex3g shows that it is difficult to stay in feasible zone with gradients. Need to stabilize!
- Can we get transfinite approach??
