...
#        names(ans)<-c("gmax","evratio","kkt1","kkt2","hev")
  ...
  if(control$trace > 0) cat("opmxtra: wrapper to extend results from opm\n")
   
  nmeth <- dim(ansout)[1]
  if (nmeth < 1) stop("No methods!")
  ncol <- dim(ansout)[2]
  npar <- length(ansout[1,]$par)
  method <- rownames(ansout)
  cnames <- colnames(ansout)
