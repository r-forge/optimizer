...
Done: in nlsr and Deriv, does a new derivative rule replace existing one? Warning?  
DM:  in newDeriv, a warning is now issued
if the rule is changed.  In Deriv, no warning is issued.

Done: Change name of Deriv and fnDeriv in nlsr to Drv, fnDrv to avoid conflict and allow
simultaneous load. Simplify -> Smplfy etc. Any others?
DM:  I'd prefer not to leave out the vowels:  Drv doesn't
look like "derivative" to me, it looks like "drive".
But the idea of avoiding conflict is good.  I have changed
the names to nlsDeriv and nlsSimplify.  We can change again if we don't like those...
...
- **nlsr**: tools `nlsDeriv()`, `fnDeriv()`, and possibly ?? `model2rjfun`
...
The tool `fnDeriv` returns an expression which can
be used in several ways.

```{r}
#- Include parameters as arguments
fj.1 <- function(x, b0, b1, th) {}
body(fj.1) <- fnDeriv(parse(text="b0 + b1 * 2^(-x/th)"), c("b0", "b1", "th")) 
fj.1(x=1, b0=2, b1=3, th=4)

#- Get parameters from the environment
fj.2 <- function(x) {}
body(fj.2) <- fnDeriv(parse(text="b0 + b1 * 2^(-x/th)"), c("b0", "b1", "th")) 
b0 <- 2
b1 <- 3
th <- 4
fj.2(1)
...
fje <- fnDeriv(parse(text="b0 + b1 * 2^(-x/th)"), c("b0", "b1", "th"))
x <- 1
eval(fje)
...
nlsr::fnDeriv are intended to output an expression to compute a derivative. 
