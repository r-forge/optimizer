wrapnls <-function(formula, start, trace=FALSE, data, lower=-Inf, upper=Inf,
        control=list(),...){
#
...
##         cat("NULL lower and upper\n")
##         lower<- (-Inf)
##         upper<-Inf
##    }
##    if ((all(lower)==(-Inf)) && (all(upper) == Inf)) cat("Inf lower and upper\n")
##    if (length(lower)==1) {
##       cat("expand lower\n")
...
##    }
    ...
    if (length(lower) < npar) {
       if (length(lower)==1) lower<-rep(lower,npar)
           else stop("lower bounds wrong length")
    } 
    if (length(upper) < npar) {
       if (length(upper)==1) upper<-rep(upper,npar)
           else stop("upper bounds wrong length")
    } 
    if (trace){ 
