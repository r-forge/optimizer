...
#- BUT (JN) seems to give back quoted string, so we must parse. ?? check
...
# Note that the result we see does NOT match what follows (JN ??)
#{
#   d <- ((x - m)/s)^2
#   .d_x <- 2 * ((x - m)/s^2)
#   -(0.5 * (.d_x * exp(-(0.5 * d))))
#}
#- For some reason the intermediate variable is NOT included.??
