...
The following (rather long) script shows things that work or fail.
...
nlsrfit0
```
We can approximate what `nls()` is doing by forcing `nlsrxb()` to use a Gauss-Newton
approach.

```{r}
nlsrfit00 <- try(nlsrxb(y0 ~ aa * exp(-bb*tt) + cc, start=st, data=mydata, trace=FALSE, 
