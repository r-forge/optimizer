...
`optimr` is a wrapper package to allow the regular R `optim()` 
calling syntax to be applied when using many different optimization 
...
* to employ some of the checks and extra optimization infrastructure
of the `optextras` package;
...
the `optimr()` function. `ctrldefault.R` lists
the optimizers available via this function in the code `optimr.R`. 
...
**optimr** had its origin in the package **optimx** ??ref??
...
function `kktchk()` from package `optextras` to test the 
...
collected in my `optextras` package. The routines used in the current package are as follows.
...
Nash, John C. and Walker-Smith, Mary (1987) Nonlinear Parameter Estimation: An Integrated System in BASIC",
New York: Dekker, now available online with software at https://archive.org/details/NLPE87plus.
