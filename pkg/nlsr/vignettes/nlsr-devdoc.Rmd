...
function `nlsrfb` -- does let us prepare residuals and Jacobians of complexity
...
```{r, error=TRUE, warning=TRUE, messages=TRUE}
...
to get the interpolating function, first with `nls()`, then with `nlsrxb()`. In the
...
nlsfit0
library(nlsr)
nlsrfit0 <- try(nlsrxb(y0 ~ aa * exp(-bb*tt) + cc, start=st, data=mydata, trace=FALSE))
nlsrfit0
```
...
minimizer through `nlsrfb`. Alternatively, we can do the modelling
via the `nlsrxb` function that
...
carried out by a common routine (`nlfb`). Generally `nlsrxb` is a 
...
Need to make it clear which package is in use
