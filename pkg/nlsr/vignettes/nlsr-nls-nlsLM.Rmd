...
try(nlsy0t0x <- nls(formula=y1~a*(t0^b), start=start1, data=edta, control=nls.control(maxiter=10000)))
cprint(nlsy0t0x)
...
We could, of course, try setting up a different formula, since the "residuals" for
nonlinear equations should be zero. Therefore we could try moving the exponential
...
We can attach the Brown and Dennis problem by applying nonlinear function minimization
