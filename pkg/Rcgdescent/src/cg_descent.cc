...
(
    ...
    long                n, /* problem dimension */
    cg_stats       *Stat, /* structure with statistics (can be nullptr) */
    cg_parameter  *UParm, /* user parameters, nullptr = use default parameters */
    double      grad_tol, /* StopRule = 1: |g|_infty <= max (grad_tol,
                                           StopFac*initial |g|_infty) [default]
                             StopRule = 0: |g|_infty <= grad_tol(1+|f|) */
    std::function<double(double *, long)> value,  /* f = value (x, n) */
    std::function<void(double *, double *, long)> grad, /* grad (g, x, n) */
    std::function<double(double *, double *, long)> valgrad, /* f = valgrad (g, x, n),
                          nullptr = compute value & gradient using value & grad */
    double         *Work  /* nullptr => let code allocate memory
                             not nullptr => use array Work for required memory
...
)
{
    ...
    Com.nf = (long) 0 ;  /* number of function evaluations */
    Com.ng = (long) 0 ;  /* number of gradient evaluations */
    iter = (long) 0 ;    /* total number of iterations */
    ...
    nrestart = (long) (((double) n)*Parm->restart_fac) ;
...
Exit:
    ...
    if ( (status > 0) && (status < 10) )
    {
        ...
        if ( Stat != nullptr ) Stat->gnorm = gnorm ;
    }
...
{
    int AWolfe, iter, ngrow, qb, qb0, status, toggle ; // PrintLevel, 
...
)
{
    ...
    Parm->maxit = std::numeric_limits<int>::max();
