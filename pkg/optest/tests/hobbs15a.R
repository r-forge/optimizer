...
for (meth in methlist){ 
   msg <- paste("Optimr attempt using ",meth," UNSCALED")
   tmp<-readline(msg)
   mydou <- optimr(start, hobbs.f, hobbs.g, method=meth, control=list(trace=1))
   print(mydou)
   msg <- paste("Optimr attempt using ",meth," SCALED")
   tmp<-readline(msg)
