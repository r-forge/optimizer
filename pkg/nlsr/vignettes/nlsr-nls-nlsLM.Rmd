...
So we can illustrate some of the issues, let us create some example data for a family
of related computational problems.
...
Let us try our example modelling `y1` against `t1`. Note that this is a zero-resitdual problem,
so `nls()` should complain or fail, which it appears to do but by exceeding the iteration limit,
which is not very communicative of the underlying issue.
...
`nls()` and `nlsLM()` return the same structure. Let us examine this for one of our 
...
such as `nlsr.predict()` below to obtain predictions. I welcome comment and discussion,
...
`nlsLM()` we use `nlsr.predict()` rather than `predict()`. As far as I can 
determine, the latter uses code `predict.nls`. 

```{r  pred1}
nudta <- colMeans(edta)
predict(nlsy1pt1, newdata=nudta)
predict(nlsLMy1pt1, newdata=nudta)
nlsr.predict(nlsry1pt1, newdata=nudta)
```
...
# now nlsr
nlsr.predict(newdata=ndata, nlsr.object=nlsrbd)


```


We could, of course, try setting up a different formula.
However, we discover that the parsing of the model formula for `nls()` and 
`nlsLM()` fails for this formulation, but `nlxb()` proceeds as usual.
...
We can try "prediction" again for `nlxb()`. The answers are, of course, rather different, as the
`nlsr.predict()` function **ONLY** evaluates the right hand side of the formula. We are **mis-applying**
the function here, and that is a reason the function has been renamed. It would be good to have more
checks on the formula, and I welcome collaboration to do this.

```{r brownden2pred}
ndata <- data.frame(t=c(5,6), y=c(0,0))
nlsr.predict(nlsrbd2, newdata=ndata)
```
