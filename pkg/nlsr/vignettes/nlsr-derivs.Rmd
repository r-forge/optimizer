...
The tool `codeDeriv` returns a function which can
be used in several ways.  `fnDeriv` wraps it in a function.
By default the arguments to the function are constructed from
the `namevec` argument.  ?? Should we default to all variables in the
expression instead?  In the example below this would include `x`.
...
more flexible:  quoted expressions, or length-1 expression vectors may also be used. ?? We
