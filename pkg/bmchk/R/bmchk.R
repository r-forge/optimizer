bmchk <- function(par, lower = NULL, upper = NULL, 
    ...
    # No lower bound exceeds an upper bound. That is the bounds
    #   themselves are
    # sensible. This condition has nothing to do with the
    #   starting parameters.
    #     maskadded: TRUE if a mask is added, FALSE if not
    # This implies very close upper and lower bounds for a
    #   parameters.
    #         See the code for the implementation.
    #     parchanged: TRUE if parameters changed, FALSE if not
    # parchanged = TRUE means that parameters are INFEASIBLE,
    #   or they would
    #         not be changed.
    #
    ########## length of vectors #########
