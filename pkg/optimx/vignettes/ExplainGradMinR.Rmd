...
$\mathbf{x}$ that provide an upper limit $s_max$.
...
- to use symbolic or automatic differentiation (@Margossian2019). R has some built-in
  tools in base R and in packages such as "Deriv" by @Deriv-pkg. However, these are, in
...
$$ s_{max} = max(\mathbf(l - x)/\mathbf{t}, \mathbf(u - x)/\mathbf{t} ) $$
...
cause elements of the parameter to very slightly violate the bound. 

?? how to simply keep within bounds.
...
described in @Fletcher70 to try to find a very compact code. At the time 8K bytes was
considered a reasonable amount of memory for a small computer. `L-BFGS-B` is based on
ideas in @Byrd95 and a quite complicated code that I feel generally uneasy discussing,
despite having worked to provide an updated version of it for R users (see @p-lbfgsb3c).
...
@HestenesCG1952 to minimizing nonlinear objective functions, @FletcherReeves1964
...
The brilliance of @FletcherReeves1964 is that the formulas for their
...
By contrast, an approach that melds the update ideas by @Dai01 has led me to
...
code. This is being fixed at the time of writing.
