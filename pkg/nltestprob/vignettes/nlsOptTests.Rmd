---
title: "A structure for building and running nonlinear equations, least squares, and optimization tests for **R**"
...
the choice of measures of success. Performance may also be greatly modified by small 
...
- be able to compare performance of particular computing platforms on a single 
...
In 2009, one of the authors (JN)
built the incomplete package `NISTopt` to allow the same problems as in `NISTnls`
...
The package **optimr** (@optimr16) can call most of the function 
...
problem. Besides `nls()`, there is `nlxb()` from **nlsr** (formerly **nlmrt**), 
`nls2()` from **nls2** (CRAN version),
...
package **nlsr** (formerly **nlmrt**) or `nlsLM` from **minpack.lm** can be specified by an **R** function
that computes the residual vector from a particular set of parameters. These functions
can also use a Jacobian function supplied by the `jacfn` argument. At the time of writing, an approximate
Jacobian will be provided by these solvers if the provided argument `jacfn` is NULL. Work is 
in process to try to provide particular derivative approximations by a mechanism similar
to the one used in package **optimrx**, where putting the name of the jacobian function in 
quotation marks indicates that a particular approximation tool is to be used. 
We can provide for use of function-based nonlinear least squares solvers by using
the name `DanielWood.res` for the residual function and `DanielWood.jac` for the Jacobian.
We assign the problem class `sumsquares` to this approach to the problem. ?? Do we wish to keep
these names??
...
is no such problem file.) ?? May want to NOT use class, but provide a list of problem types
...
Considerations 170221?? 170418

-- only specify bits that are available and let problem setup program (GUI) do the rest.

-- steal ideas from optimgui

-- allow scripting so work can be automated (and save such a script)
...
will experiment with the current approach until we discover it cannot support 
our requirements. 
...
#- ?? Explain why we need this environment.
counters <- new.env()
```
...
start the Extended Rosenbrock test function (https://en.wikipedia.org/wiki/Rosenbrock_function, 
based on @Rosenbrock60) with a set of 10 parameters each having 
...
residuals and Jacobian by the nonlinear least squares methods. In the case of package **nlsr**
(of which an earlier approach was package **nlmrt**), the Jacobian is computed 
...
- Potentially multiple sub-cases if there are choices within a method, such as the three update 
  formulas for optim::CG, or the "plinear" and "port" options for `nls()`.
  
- An unspecified number of extra cases if we set limits on the function evaluations etc., or 
  the controls such as various "convergence" tolerances, Marquardt stabilization parameters and
  adjustments, and many other choices.
...
- analytic gradients if available, else the default (internal) gradient approximation.

This last choice may be open to discussion. The (internal) gradient approximations used by
different methods are not necessarily equivalent, so it may be sensible to test with a 
reasonable but "standard" approximation such as the central finite difference approximation. 
The purpose of using a common approximation is to allow comparison of methods, rather than methods
...
be a choice if it is known to be applicable. While the computations of the complex step derivative
are relatively simple, determination that the underlying function is **analytic** is more difficult,
and many functions are not.
...
source("../inst/probfiles/XRosenbrock.prb")
...
Issues to address?? :
 ...
 ## Appendix D: Another approach
 
 Based on `optimgui`, using `gWidgets2`, could do things very differently.
 
 1) input or set (default?) the problem file directory
 
 2) provide a selector for all problems of type `.prb`
 
 3) read selected files and save their "pieces"
 
    - problem name (should match the file name -- keep this as a cross check)

    - data file options -- one type of variant of the problem
    
    - bounds or mask constraints choices -- another variant setting
    
    - start options -- yet another variant. This will also set the `n` or 
    number of parameters.
  
    - function definitions or formula. These are .f, .g, .formula, .jac, .res, 
    and possibly other types of structures. We will AVOID .f if there is .res,
    UNLESS there is a separate evaluation mechanism.
    
    - How can we distinguish nonlinear equations from nonlinear least squares as
    defined by .res functions. Or is there a real difference?
    
    - Build a consistent "problem instance description/name" made up of problem name,
    data choice, constraint choice, start choice.
    
    - Choose one or more solvers, checking if they are appropriate. What checks
    are needed.
    
    - If appropriate, select variant(s) of the solver(s), or set controls.
    
    - Create consistent name or description of "solver instance".

    - Extract platform description.
    
    - Set up "playlist" for runs. May want to actually create a detailed list
    then step through them, rather than use control loop. The list could be
    broken up and run during idle or low-activity times.
