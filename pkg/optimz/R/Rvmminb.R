  ...
  if ((class(f) == "try-error") | is.na(f) | is.null(f) | is.infinite(f)) {
    ...
    while (keepgoing) { ## main loop -- must remember to break out of it!
      if (ilast == ig) { # reset the approx. inverse hessian B to unit matrix
          ...
          if (ctrl$trace > 2) cat("Reset Inv. Hessian approx at ilast = ", ilast, "\n")
      }
      fmin <- f
      if (ctrl$trace > 0) cat(" ", ifn, " ", ig, " ", fmin, "\n")
        ...
        while ((f >= fmin) && changed && (!accpoint)) {
          ...
          for (i in 1:n) { # loop on parameters -- vectorize??
            if ((bdmsk[i] == 1) && (t[i] != 0)) {
              ...
              if (ctrl$trace > 2) cat("steplength, trystep:", steplength, trystep, "\n")
          ...
          if (ctrl$trace > 1) cat("reset steplength=", steplength, "\n")
          ###  }  # end if bounds
          # end box constraint adjustment of step length
          ...
          if (changed) {
            ...
            if (ctrl$trace > 1) cat("Unchanged in step redn \n")
          }
      ...
      if (accpoint) {
        ...
        for (i in 1:n) { ## Reactivate constraints?
          if (bdmsk[i] == 1) { # only interested in free parameters
            ...
            if ((bvec[i] - lower[i]) < ceps * (abs(lower[i]) + 1)) {
              ...
              if (ctrl$trace > 2) cat("(re)activate lower bd ", i, " at ", lower[i], "\n")
              bdmsk[i] <- -3
            }  # end lower bd reactivate
            if ((upper[i] - bvec[i]) < ceps * (abs(upper[i]) + 1)) {
              # are we near or above upper bd
              if (ctrl$trace > 2) cat("(re)activate upper bd ", i," at ", upper[i], "\n")
              bdmsk[i] <- -1
            }  # end lower bd reactivate
        ...
        if (ctrl$trace > 0) cat("ig=",ig,"  gnorm=",gnorm,"  ")
        if (gnorm < (1 + abs(fmin))*ctrl$eps*ctrl$eps ) {
          if (ctrl$trace > 1) cat("Small gradient norm\n")
          keepgoing <- FALSE
          conv <- 2
          break
        }
        ...
        if (D1 > 0) {
          ...
          if (ig == ilast+1) {
            if (ctrl$stopbadupdate) keepgoing=FALSE # stop on update failure for s.d. search
            if (ctrl$trace > 2) cat("keepgoing = ",keepgoing,"\n")
            conv <- 3
          }
        ...
        if ((ig == ilast) && (ig > 2)) {
          ...
          if (ctrl$trace > 0) cat("Reset to gradient search\n")
        }  # end else ig != ilast
      }  # end else no accpoint
    }  # end main loop  (while keepgoing)
    if (ctrl$maximize) fmin <- (-1) * fmin
    if (ctrl$trace > 0) cat("Seem to be done Rvmminb\n")
