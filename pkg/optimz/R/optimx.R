...
#  optcfg$ctrl$have.bounds<-optcfg$have.bounds # to pass boundedness
  ...
  output <- structure(ansout, details = details, maximize = optcfg$ctrl$maximize,
            npar = optcfg$npar, follow.on=optcfg$ctrl$follow.on,
            class = c("optimx", "data.frame"))
  output # as per Gabor G. 140811 allows   trace(optimx, exit = quote(print(output)))
} ## end of optimx
