...
it here, but we did perform a quick check with package \texttt{numDeriv} that the gradient is 
correctly computed for the starting vector $x0$.

```{r polygradu}
```

There are a number of unconstrained optimization methods, and the \texttt{optimz} package lets
us try them out all at once.

```{r polyexuall}        
```

We note that the only bounds constrained minimizer in \texttt{R}'s base function \texttt{optim()} is
...
Let us try all the bounds-constrained methods in \texttt{opm} from package \texttt{optimz.

```{r polyexallb}        
```

Graham reports largest area is approximately 0.674981.