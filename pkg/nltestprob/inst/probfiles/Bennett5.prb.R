...
Bennett5.formula <- ( y ~ b1*(b2+x)^(-1/b3) )
...
print(Bennett5nlxb1)
Bennett5nlxb2 <- try(nlxb(start=start2, formula=Bennett5.formula, trace=TRUE, data=mypdata))
print(Bennett5nlxb2)

library(optimr)
library(numDeriv) 

cat("Test Bennett5 funcion")

B5f1 <- Bennett5.f(start1)
cat("At start1 =",B5f1,"\n")
B5g1 <- Bennett5.g(start1)
B5gn1 <- grad(Bennett5.f, start1)
cat("max abs deviation between analytic and numerical gradient =", max(abs(B5gn1-B5g1)),"\n")


B5f2 <- Bennett5.f(start2)
cat("At start1 =",B5f2,"\n")
B5g2 <- Bennett5.g(start2)
B5gn2 <- grad(Bennett5.f, start2)
cat("max abs deviation between analytic and numerical gradient =", max(abs(B5gn2-B5g2)),"\n")
