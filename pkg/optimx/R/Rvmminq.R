Rvmminq <- function(par, fn, gr = NULL, lower = NULL, 
  upper = NULL, bdmsk = NULL, control = list(), ...) {
  #
  #  Author:  John C Nash
  #  Date:  April 2, 2009; revised July 28, 2009, May 21, 2012, Jan 8, 2015, 2018-11-19
  #
  ## An R version of the Nash version of Fletcher's Variable
  #   Metric minimization 
  # This uses a simple backtracking line search WITH quadratic inverse interpolation
  ...
  if (bounds) { 
    ...
    ############## end bounds check #############
    ans <- Rvmminqb(par, fn, gr, lower = lower, 
        upper = upper, bdmsk = bdmsk, control = control, ...)
    } else {
       ans <- Rvmminqu(par, fn, gr, control = control, ...)
    } #   return(ans) 
}  ## end of Rvmmin
