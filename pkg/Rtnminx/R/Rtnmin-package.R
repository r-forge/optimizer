...
#   print(p)
   ...
   for (itcnt in 1:maxit) {
      xt <- x + alpha1*p 
      fg <- sfun(xt, fonly=TRUE, ...) # Note: added dots 140902
      ft<-fg
      gt <- rep(0,length(x)) # to ensure defined
#      gt<- attr(fg,"gradient") # may simplify later
      if (ft < f) {
         ierror <- 0
         xnew   <- xt
         fnew   <- ft
         ft <- sfun(xt, ...) # to get gradient !!! NOT EFFICIENT
         gt<- attr(ft,"gradient") # may simplify later
         gnew   <- gt
