...
g(x) = 
\left( \begin{matrix}
\sum_{l=1}^3 u_{l1} e_l x_l - K_1 \\
\sum_{l=1}^3 u_{l2} e_l x_l - K_2 
\end{matrix} \right).
$$
Derivatives are given by
$$
...
\subsection{General case\label{app:nseq:gencase}}
...
$$
D_a(z) = \diag[ a^1(x, \lambda^1) , \dots , a^N(x, \lambda^N) ] 
\txtm{and}
D_b(z) = \diag[ b^1(x, \lambda^1) , \dots , b^N(x, \lambda^N) ],
$$
...
\subsection{Shared constraint case\label{app:nseq:jointcase}}
...
\hline 
	\begin{matrix}	
	- D_N^a(x, \mu) \Jac_{x_1} h(x) & \dots & - D_N^a(x, \mu) \Jac_{x_N} h(x) \\
	\end{matrix}		
&
	\begin{matrix}	
	0  & \dots &  0 \\
