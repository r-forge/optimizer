...
aspgs <- spg(par=st, fn=ssum, project="projectLinear",  projectArgs=list(A=Amat, b=b, meq=meq))

proj<-proj<-function(x) x/sum(x)
cat("Try spg with ssums rather than ssum \n")
assums<-spg(st, ssums, project=proj)
assums
cat("with just ssum\n")
assum<-spg(st, ssum, project=proj)
assum
