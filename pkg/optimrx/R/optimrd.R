...
#  cat("control$have.bounds =",control$have.bounds,"\n")
  ...
  npar <- length(par)
  defctrl <- ctrldefault(npar) # get control vector defaults

  idx <- 1:npar # index of active parameters
...
#   }
   ...
   # New approach to masks 160711 -- use idx in efn egr
   mskd <- which(lower >= upper)
   idx <- idx[which(lower < upper)]
   if (control$trace > 0) {
       if (length(mskd) > 0) {
...
# 160615 -- decided to abandon nloptr in optimz
  ...
  efn <- function(spar, ...) {
      ...
      # also rely on idx being defined. BUT -- we need to transform so use
      # xpar -- the parameters which are NOT masks, then xpar --> spar
      par <- spar*pscale
      val <- fn(par, ...) * fnscale
  }
