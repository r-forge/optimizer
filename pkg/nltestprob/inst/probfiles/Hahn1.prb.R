...
Hahn1.formula <- ( y ~ (b1+b2*x+b3*x**2+b4*x**3) / (1+b5*x+b6*x**2+b7*x**3) )

#- setup

library("NISTnls", character.only=TRUE)
mypdata <- eval(parse(text=data("Hahn1")))
...
hahn1.setup<-function() {
  ...
  start1 <- c(b1 = 10, b2 = -1, b3 = 0.05,
            b4 = -0.00001, b5 = -0.05, b6 = 0.001, b7 = -0.000001)

  
  start2 <- c(b1 = 1, b2 = -0.1, b3 = 0.005, b4 = -0.000001,
...
Hahn1nlxb1 <- nlxb(start=start1, formula=Hahn1.formula, data=mypdata, trac=TRUE)
print(Hahn1nlxb1)

Hahn1nlxb2 <- nlxb(start=start2, formula=Hahn1.formula, data=mypdata, trac=TRUE)
print(Hahn1nlxb2)

library(optimr)

Hahn1opm1fwd <- opm(start1, hahn1.f, "grfwd", method="ALL")
summary(Hahn1opm1fwd, order=value)

Hahn1opm2fwd <- opm(start2, hahn1.f, "grfwd", method="ALL")
summary(Hahn1opm2fwd, order=value)
