...
Here's the same example, with a transform that does two things:  it
reports whenever $x < 0$, and it fixes $y$ at the value 10.
...
transform <- local({
  ...
  forward <- function(par) {
    x <- par[1]
    if (x < 0) message("x = ", format(x))
    x
  }
  inverse <- function(par1) {
    c(par1, yval)
  }
  fn1 <- function(par1) fn(inverse(par1))
  list(forward = forward, inverse = inverse, fn1 = fn1)
})
