...
{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error in eval(expr, envir, enclos): could not find function "{}opm"{}}}\begin{alltt}
   \hlstd{ao}\hlkwb{<-}\hlkwd{summary}\hlstd{(ans,}\hlkwc{order}\hlstd{=value)}
\end{alltt}


{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error in summary(ans, order = value): object 'ans' not found}}\begin{alltt}
\hlkwd{print}\hlstd{(ao)}
\end{alltt}


{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error in print(ao): object 'ao' not found}}\end{kframe}
\end{knitrout}
...
{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error in summary(a5, order = value): object 'a5' not found}}\begin{alltt}
\hlkwd{summary}\hlstd{(a5g,}\hlkwc{order}\hlstd{=value)}
\end{alltt}


{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error in summary(a5g, order = value): object 'a5g' not found}}\begin{alltt}
\hlkwd{summary}\hlstd{(a5gb,}\hlkwc{order}\hlstd{=value)}
\end{alltt}


{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error in summary(a5gb, order = value): object 'a5gb' not found}}\begin{alltt}
...
is a similar issue with the $n-1$ parameters near zero. Negative 
values will cause difficulties. 
...
{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error in library(BB): there is no package called 'BB'}}\begin{alltt}
...
{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error in proj2((ans100\$par)): object 'ans100' not found}}\end{kframe}
\end{knitrout}

In the above, we note that the transformation is embedded into the objective function, 
so we could run any of the optimizers in \pkg{optimrx} as follows. This takes some time, 
as the derivative-free
