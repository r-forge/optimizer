...
# Restrict list of methods if we have bounds
  if (any(is.finite(c(lower, upper)))) { ctrl$have.bounds<-TRUE # set this for convenience
  } else { ctrl$have.bounds <- FALSE }

  if (is.null(method)) {
    if (ctrl$have.bounds) { method <- c("lbfgsb3", "bobyqa", "Rvmmin", "Rcgmin", "nmkb")}
