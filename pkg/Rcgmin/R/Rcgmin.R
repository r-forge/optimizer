...
# set default masks if not defined
  if (is.null(bdmsk)) {
       bdmsk<-rep(1,n) # all parameters free initially
  }
...
####################################################################
  ...
  while (keepgoing && notconv) {  # main loop -- must remember to break out of it!!
    ...
    while (keepgoing && (cycle < cyclimit)) { ## cycle loop
      ...
      gradsqr<-sum(g*g)
      if (trace > 1) {
          cat("Gradsqr = ", gradsqr," g1, g2 ", g1, " ", g2," fmin=",fmin,"\n")
          #!! tmp<-readline()
      }
      #!! gtc<-sum(g*c) #!! to check below -- but remove later
      #!! cat("t2=",sum(t*t),"\n")
      #!! if (gtc > 0.2*gradsqr) cat("Powell restart FLAGGED g*c=",gtc,"\n") #!!
      c<-g # save last gradient
      g3<-1.0 # !! Default to 1 to ensure it is defined -- t==0 on first cycle
      if (gradsqr > tol*(abs(fmin)+reltest) ) {
         if (g2 > 0.0) {
            betaDY<-gradsqr/g2
            betaHS<-g1/g2
            g3 <-max( 0.0, min(betaHS, betaDY) ) # g3 is our new 'beta' !! Dai/Yuan 2001, (4.2)
         } 
      } else {
	 msg<-paste("Very small gradient -- gradsqr =",gradsqr,sep=' ')
      ...
      if (g3 == 0.0 || cycle>=cyclimit) {  # we are resetting to gradient in this case 
          if (trace > 0) {
...
#!!    oldstep = setstep * newstep # change to newstep for steplength
