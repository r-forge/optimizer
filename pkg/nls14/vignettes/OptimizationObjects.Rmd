...
*  optstart() 
    - Create the local oe2015 environment (JN??: Is this name sufficiently unique, or
        should we use a timestamp and save the name. At the moment I prefer the simple
        name, as it lets power users employ the content of the environment, which is
        still to be defined.)
*  makeufn()   
    - using **maximize**, fn, gr, hess, parscale, set up a ufn() object for use by optimization routines
    - output ufn() (JN??: Other name choices? e.g., ofn())
*  makeoptobj
    - create the optimization object
    - contents of this object need to be delineated
*  setoptctrl()  (JN??: currently ctrldefault())  
    ...
    - Called with `method=methname`, where `methname` is a character string for a single method,
      sets default controls for that method.
    - output is a list
...
*  optxsetup()
    - Intended to call some of the routines above and ensure that inputs are sane
    - we could make this common for `optim()` and `optimx()` and `run.optimization()`,but would need to take
    care with the different options and multiple methods for optimx.
*  optcheck()
    - verify bounds are admissible
    - check parameters are feasible (within bounds, or else moved to bounds)
*  optmethcomp()
    - current optimx() idea of comparing several methods for optimization and prepare results in a 
      dataframe
    - Calls   optmeth.setup
              optcheck
              optmeth.run  -- calls optim, or possibly its internal routines
              optmeth.post -- does the post run kkt and related tests
*  optim()
