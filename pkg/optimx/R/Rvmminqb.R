Rvmminqb <- function(par, fn, gr = NULL, lower = NULL, 
  upper = NULL, bdmsk = NULL, control = list(), ...) {
  #
  #  Author:  John C Nash
  #  Date:  , April 14, 2018 -- update to fix infinite loop
  #           Nov 19, 2018 to add quadratic inverse interp.
  #
...
#    reltest=100.0, stopbadupdate = TRUE)
  ...
  reltest <- ctrl$offset # 181119 to match ctrldefault()
...
#################################################################
  ...
  if (nolower && noupper && all(bdmsk == 1)) { 
     bounds = FALSE
     stop("Do not use Rvmminqb() without bounds.")
  } else { bounds = TRUE }
...
## Set working parameters (See CNM Alg 22)
  if (trace > 0) {
     cat("Rvmminqb -- J C Nash 2009-2015 - an R implementation of Alg 21\n") }
  ...
  if (is.null(gr)) {  # if gr function is not provided STOP 
    stop("A gradient calculation (analytic or numerical) MUST be provided for Rvmminqb")
  }
...
#              if ((kf == 1) && (f < fmin)) { # try quad point
                ...
                bq <- par + sq * t
                if (trace > 2) { cat("bq:"); print(bq) }
                changed <- (!identical((bvec + reltest), (par + reltest)) )
                if (changed) {
                  fq <- try(fn(bq, ...))
                  if (class(fq) == "try-error") f <- .Machine$double.xmax
                  if (maximize) fq <- -fq
                  if (trace > 2) cat("fq=",fq," lower = ",(fq < fmin),"\n")
                  kf <- kf + 1
                  ifn <- ifn + 1
                  if (ifn > maxfeval) {
                    msg <- "Too many function evaluations"
...
#                }
    ...
    if (trace > 0) {
       cat("Seem to be done Rvmminqb\n")
       cat(msg,"\n")
    }
    #    msg <- "Rvmminqb appears to have converged"
    counts <- c(ifn, ig)
